...
The webpage also reads the Cloudflare Worker live cache every 15 seconds:

- `https://worldcup-tronky-live.eavileslino.workers.dev/scores`

When that live request works, the browser recalculates the table immediately using `docs/data/predictions.json`. If the request fails, the page falls back to the JSON files generated by GitHub Actions.
